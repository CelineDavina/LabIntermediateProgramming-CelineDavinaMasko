...
def get_board_size():
    ...
    while True:
        try:
            size = simpledialog.askinteger("Input", "Enter the board size:", minvalue=3)
            if size is None:
                messagebox.showinfo("Exit", "No size entered. Exiting the game.")
                root.destroy()
                exit()
            return size
        except ValueError:
...
def get_win_combo(board_size):
    root = tk.Tk()
    root.withdraw()  # Hide the root window
    while True:
        try:
            combo = simpledialog.askinteger("Input", f"Enter the win combo size (between 2 and {board_size}):", minvalue=3, maxvalue=board_size)
            if combo is None:
            ...
            messagebox.showerror("Invalid input", f"Please enter a valid integer between 3 and {board_size}.")

class TicTacToeGame:
    def __init__(self, players=DEFAULT_PLAYERS, board_size=3, win_combo=3):
    ...
    def play(self, event):
        ...
        if self._game.is_valid_move(move):
            ...
            if self._game.is_tied():
                self._update_display(msg="Tied!", color="pink")
...
def main():
    ...
    win_combo = get_win_combo(board_size)
